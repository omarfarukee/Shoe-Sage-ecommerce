...
export default function Sneakers() {
  ...
  const totalFormalItems = filteredShoeData.filter((shoe) => shoe?.category === "sneakers").length;
  const totalPages = Math.ceil(totalFormalItems / itemsPerPage);
  
  const currentData = filteredShoeData
    .filter((shoe) => shoe?.category === "sneakers")
  ...
  return (
    <div className="overflow-hidden">
      {cartItems.length !== 0 &&
            <NavLink to="/cart">
              <div className="fixed hidden lg:block bottom-4 right-4 bg-white p-4 rounded-full shadow-lg border z-40">
      ...
      >
        <div className="w-[250px] lg:h-[100vh] hidden lg:block pt-2 sticky">
        ...
        <div className="lg:w-[1200px]">
          <p className="text-titleMd ml-5 mt-5 mb-5">Sneakers Collections</p>
          <div className="w-full h-16 bg-[#f7f7f7] items-center flex justify-between px-10 mb-3 mx-3">
            <p className="flex items-center lg:gap-5 gap-2">View as: <AiOutlineTable className="lg:text-titleSm" /></p>
            <select
          ...
          <div className="flex flex-wrap gap-6 w-[100%] justify-center">
           ...
           { currentData.length ===0 && <div className="text-titleLg mt-5">
              No product fround
            </div>
}
            {currentData?.filter((shoe) => shoe?.category === "sneakers").map((shoe) => (
